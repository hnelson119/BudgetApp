...
def test_production_settings_pin_exact_private_ingress_and_proxy_boundary() -> None:
    ...
    assert "Production requires one exact lowercase Tailscale HTTPS hostname." in settings_file
    assert "Production CSRF origins must contain only the exact Tailscale HTTPS origin." in (
        settings_file
    )
    assert 'MIDDLEWARE.insert(0, "core.middleware.ProxyBoundaryMiddleware")' in settings_file
    assert 'MIDDLEWARE.insert(2, "core.middleware.HostBoundaryMiddleware")' in settings_file
    assert "USE_X_FORWARDED_HOST = False" in settings_file
    assert "SESSION_COOKIE_DOMAIN = None" in settings_file
    assert "CSRF_COOKIE_DOMAIN = None" in settings_file
