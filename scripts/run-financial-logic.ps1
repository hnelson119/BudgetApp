...
if (-not (Get-Command docker -ErrorAction SilentlyContinue)) {
    ...
    & wsl.exe sh -lc "cd '$wslProjectRoot' && sh ./scripts/run-financial-logic.sh"
    exit $LASTEXITCODE
}

$composeFile = Join-Path $projectRoot "compose.pentest.yaml"
$projectName = "budgetapp-financial-logic"
$composePrefix = @(
    "compose", "-p", $projectName, "-f", $composeFile, "--profile", "financial"
)

& docker @composePrefix down --volumes --remove-orphans
if ($LASTEXITCODE -ne 0) {
    throw "Unable to clear the fixed disposable financial-logic project."
}

try {
    & docker @composePrefix up --build --wait pentest-web
    if ($LASTEXITCODE -ne 0) {
        throw "The disposable financial-logic application failed to start."
    }
    ...
    & docker @composePrefix run --build --rm --no-deps pentest-financial-logic
    if ($LASTEXITCODE -ne 0) {
        throw "The disposable financial-logic probes failed."
    }
}
finally {
    & docker @composePrefix down --volumes --remove-orphans
    if ($LASTEXITCODE -ne 0) {
        Write-Error "The fixed disposable financial-logic project could not be fully removed."
    }
}

Write-Host "Disposable financial-logic probes completed."
