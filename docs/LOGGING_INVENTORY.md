...
Status: implemented inventory; release and separate-destination work pending
...
Together they implement the repository-deliverable portion of ASVS `v5.0.0-16.1.1`. They do not
claim a release-candidate pass. The Django security stream remains in the same host-local Docker log
store as operational records, so `v5.0.0-16.4.3` remains not started until a logically separate
protected destination is selected and tested.
...
| Django security | Authentication, MFA/recovery, session, rejected import, and sensitive export outcomes; tagged redacting JSON | Separate Python handler but same Docker store; deployment administrators | 20 MiB × 5 files; separate off-host copy still missing |
...
The JSON inventory is authoritative for exact language. Its four event groups are generated from
source literals and validated against the repository: 2 Django operational events, 23 Django
security events, 83 protected audit actions, and 35 structured maintenance events. Adding, removing,
or renaming a literal event without updating the inventory fails the local and CI gate.
...
4. On the selected Linux VM, inspect Docker's effective driver/options for every service, journald
...
6. Attempt representative credential, email, long-number, financial, exception, import, and request
   ...
   JSON encoding, request correlation, no raw access logs, and no raw security report in Git.
7. Review all known gaps. The inventory must continue to say that the security stream is co-located
   until an independent destination, restricted readers, delivery-failure alert, retention, and
   incident escalation path are implemented and tested.
...
Docker and host journals are restricted to deployment administrators; Docker group membership is
root-equivalent. Household application users have no operational-log access. They may see only
household-scoped audit summaries, with recent password-plus-MFA verification required for detailed
audit access and export. The runtime database role can append through the controlled function but
cannot update/delete audit data; the integrity job has a separate read-only identity.

During an incident, establish a time window and correlation IDs, preserve sources before rollover,
and compare application operational/security records, protected audit chains/checkpoints,
PostgreSQL, Docker, systemd, SSH/sudo, UFW, Tailscale, backup/restore, and provider configuration
records. Treat missing or contradictory records as a finding. Never paste raw evidence into a ticket,
chat, pull request, or Git commit.

## Known gaps

The inventory is complete for the current stack but does not create controls that do not exist.
Django security logs remain on the same VM and in the same Docker store as operational logs. No
SIEM, remote alert pipeline, or independent operational archive exists. The host's effective
journald policy and provider/account settings remain release-only observations. Optional Tailscale
network flow logging is not enabled or assumed. These gaps stay open until their own implementation
and verification slices are complete.
